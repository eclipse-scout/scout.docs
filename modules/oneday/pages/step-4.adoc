...
Before we start with the actual implementation of the form, we introduce the layouting concepts of the Scout framework.
...
According to the form layout defined in <<Designing the Person Form>>, the city field and the country field will be located on the same logical row and in the same cell of the logical grid.
