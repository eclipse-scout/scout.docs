...
Because invoked in another thread, this method optionally accepts a xref:technical-guide:scout-classic/RunContext.adoc[]] to be applied when being invoked.
...
Job manager can be configured with properties starting with `scout.jobmanager`. See xref:technical-guide:scout-classic/Platform.adoc#sec-app.configProperties[Scout Config Properties].
