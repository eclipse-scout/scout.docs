...
Use this translator to work with runtime exceptions. When working with xref:technical-guide:common-concepts/RunContext.adoc[]] or xref:technical-guide:common-concepts/JobManager.adoc#ifuture[IFuture], some methods optionally accept a translator. If not specified, this translator is used by default.
