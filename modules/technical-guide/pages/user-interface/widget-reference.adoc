...
In order to provide the proposals you can either use a xref:technical-guide:working-with-data/LookupCall.adoc[] or a `Code Type`.
...
For a more detailed example see xref:howtos:scout-js/Chart-HowTo.adoc[How to Create a Chart for Scout JS] resp. xref:howtos:scout-classic/Chart-HowTo.adoc[How to Create a Chart for Scout Classic]
