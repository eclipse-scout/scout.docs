...
Use this translator to work with runtime exceptions. When working with xref:technical-guide:RunContext.adoc[]] or xref:technical-guide:JobManager.adoc#ifuture[IFuture], some methods optionally accept a translator. If not specified, this translator is used by default.
