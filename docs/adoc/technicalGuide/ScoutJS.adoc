...
2. The widget may be extended (see chapter <<Object Factory>> for details).
...
=== Object Factory
As seen in the chapter <<Creating a Widget>> a widget may be created using `scout.create`. When using this function, the call is delegated to the `ObjectFactory`.
