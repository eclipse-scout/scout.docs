...
Note: *<<PlatformError>>s should never be catched by business logic!* See <<Scout Runtime Errors>> for more information.
...
By default, a <<ProcessingException>> is logged according to its severity, a <<VetoException>>, <<ThreadInterruptedError>> or <<FutureCancelledError>> logged in _DEBUG_ level, and any other exception logged as an _ERROR_. If running client side, exceptions are additionally visualized and showed to the user.
