...
Data objects are Scout beans, which are used as data transfer objects for synchroneous REST and asynchroneous MOM interfaces.
...
The Scout module `org.eclipse.scout.rt.jackson` provides a default implementation serializing data objects from and to JSON using the https://github.com/FasterXML/jackson[Jackson libary].
...
NOTE: Declaring a logical type name using the `@TypeName`annotation is highly recommended to avoid a 1:1 dependency between the serialized JSON String and the fully classified class name. A stable type name is required in order to be able to change the data object structure without breaking the API.
...
Instead of data objects, a REST or MOM interface could be build using simple plain old Java objects (POJOs).
Compared to POJOs a Scout data object offer additional support and convenience when working with attributes.
...
For a complex hierarchy of data objects the base class may be abstract and extends the `DoEntity` class, further subclasses extends the abstract base class.
...
The base class `DoEntity` does not provide a default implementation of `equals()` and `hashCode()`, since a data object may contain any types of data which must not provide a useful `equals()` implementation.
If a data object should be used as map key, a custom `equals()` and `hashCode()` method taking into account the relevant attributes should be added.
For unit tests there is a utility providing different generic compare methods for data objects, see section <<sec-unit-testing>>.
...
Commonly used are a set of assert methods (e.g. `assertDoEntityEquals()`) for testing data objects for (deep) equality.
