...
|List
|Config Property
|`scout.dev.scriptfile.persist.key`
|Specifies a key to store the keys (HttpCacheKey) scripts in development cache. The keys of the last application start in dev mode will be stored in the user.home/.eclipse/org.eclipse.scout.dev/scriptfile_cache_{key}.obj. For the next run in dev mode the keys stored under this key are preloaded.
|String
|Config Property
|`scout.dev.scriptfile.rebuild`
|Specifies if scriptfiles (*.js, *.less) should be rebuilt only when the files change or with every page reload.
