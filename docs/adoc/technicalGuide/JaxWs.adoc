...
JAX-WS Scout integration is prepared to run with different implementors. Unfortunately, some implementors do not implement the JSR exactly, or some important functionality is missing in the JSR. To address this fact without loosing implementor independence, the delegate bean <<JaxWsImplementor,JaxWsImplementor>> exists.
...
|Specifies the annotation processor to be used (<<JaxWsAnnotation,JaxWsAnnotation>>) and dependent artifacts
|===
...
The field 'authentication' on `WebServiceEntryPoint` configures what authentication mechanism to install on the webservice endpoint, and in which `RunContext` to run authenticated webservice requests. It consists of the <<IAuthentication,IAuthentication>> to challenge the client to provide credentials, and the <<ICredential,ICredential>> to verify request's credentials against a data source.
...
See <<IAuthentication,IAuthentication>> for more information.
...
See <<ICredential,ICredential>> for more information.
...
TIP: If using container based authentication (authentication enforced by the application server), use <<ContainerBasedAuthentication,ContainerBasedAuthentication>> as authentication method, and do not configure a credential verifier.
