...
The class EventListenerList has poor performance with large numbers of listeners and add / remove operations.
...
[NOTE]
====
Consider refactoring the use of EventListenerList by one of the new alternatives.
====
...
By default (searchRequired=false) all existing proposals are shown if no search constraint has been typed. But if the property is set to `true`, the Smartfield only shows proposals if a search constraint is available.
...
In Java the property can be set using `org.eclipse.scout.rt.client.ui.form.fields.smartfield.ISmartField.setSearchRequired(boolean)` or `org.eclipse.scout.rt.client.ui.form.fields.smartfield.AbstractSmartField.getConfiguredSearchRequired()`.
In JavaScript the property can be set using `scout.SmartField.setSearchRequired(boolean)`.
...
With the new release 8.0 Scout supports hierarchical tables. The property `parentKey` on the `IColumn` is responsible for the linking between parent and child row.
...
The scout multi-language text support mainly is ITextProviderService, AbstractDynamicNlsTextProviderService and the convenience accessor TEXTS.
This facility was moved from `org.eclipse.scout.rt.shared` to `org.eclipse.scout.rt.platform.text`
