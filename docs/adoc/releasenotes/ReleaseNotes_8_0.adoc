...
== HttpException deprecated and replaced by RemoteSystemUnavailableException
The `org.eclipse.scout.rt.shared.servicetunnel.HttpException` was replaced by a generalized `org.eclipse.scout.rt.platform.exception.RemoteSystemUnavailableException`.
The exception is thrown within the UI server, when a call to the Scout backend server (through service tunnel) returns a status code < 200 or >= 299.
The `RemoteSystemUnavailableException` is finally displayed as _network error_ by the global `org.eclipse.scout.rt.client.services.common.exceptionhandler.ErrorPopup`.
