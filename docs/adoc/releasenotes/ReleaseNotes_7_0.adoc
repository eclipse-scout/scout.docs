...
== New Property Auto Optimize Column Max Width (since 7.0.300)
The new property `autoOptimizeMaxWidth` determines the maximum width columns will not grow to when they are auto optimized when `autoOptimizeWidth` is set to true. The user may still manually make columns wider by dragging or double-clicking the column separators.
