...
* ICompositeField.java `setFields` and the already existing `addField`, `removeField` methods which does not throw anymore an exception when a form is already initialized.
