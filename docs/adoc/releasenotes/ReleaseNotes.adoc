...
===== New Possibility to Reload the Table.
The new table may show a link to let the user reload the table. To make this work the table needs to provide a `IReloadHandler`. Default reload handlers are `PageReloadHandler` and `TableFieldReloadHandler`.
...
==== Tree
===== New property defaultIconId
Compared to the property `iconId` the difference is as follows: _DefaultIconId_ is used as default for all tree nodes that don't have an icon on their own. _IconId_ may be used in the same way as the title, e.g as outline icon.
...
move org.eclipse.scout.rt.server.commons.authentication.ConfigFileCredentialVerifier to org.eclipse.scout.rt.platform.security.ConfigFileCredentialVerifier
move org.eclipse.scout.rt.server.commons.authentication.ICredentialVerifier to org.eclipse.scout.rt.platform.security.ICredentialVerifier
move org.eclipse.scout.rt.server.commons.authentication.IPrincipalProducer to org.eclipse.scout.rt.platform.security.IPrincipalProducer
move org.eclipse.scout.rt.server.commons.authentication.SimplePrincipalProducer to org.eclipse.scout.rt.platform.security.SimplePrincipalProducer

=== URL Hints (security)
The new config property `scout.urlHints.enabled` controls if the URL parameters ?cache, ?compress, ?minify, ?debug, ?inspector are active.
By default these are not enabled. This means, that by default it is not possible (for an attacker) to disable minify and get the original javascript sources.
