
ifndef::finaldoc[]
include::../_initDoc.adoc[]
endif::finaldoc[]

//fallback for safe mode == secure:
:imgsdir: ../../imgs
:codedir: ../../../code
ifndef::mdledir[:mdledir: .]
:experimental:

:docroot: https://eclipsescout.github.io/{doc-short-version}/
:techdoc: https://eclipsescout.github.io/{doc-short-version}/technical-guide.html
:migrationguide: https://eclipsescout.github.io/{doc-short-version}/migration-guide.html

= Overview

Eclipse Scout 6.0 is part of the Eclipse Neon release. The latest stable version is _Neon.3 (6.0.300.1)_. See also https://wiki.eclipse.org/Simultaneous_Release for the release schedule.

* Download SDK: link:http://www.eclipse.org/downloads/packages/eclipse-scout-developers/neon2[Eclipse for Scout Developers]
* Runtime on Maven Central: http://search.maven.org/#search%7Cga%7C1%7Cg%3A%22org.eclipse.scout.rt%22%20AND%20v%3A%226.0.300.1%22[6.0.300.1]


//-----------------------------------------------------------------------------
// This file contains the release notes
//-----------------------------------------------------------------------------
include::ReleaseNotes_6_0.adoc[]
