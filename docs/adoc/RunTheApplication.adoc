...
In case there are no launch configurations available you can also find the .launch files in the projects `helloworld.server.app.dev` and `helloworld.ui.html.app.dev`.
They can be started using the context menu `Run As`. This will also add them to the `Run` toolbar button for the next time.
