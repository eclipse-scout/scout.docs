...
You can get a Java 11 runtime from https://adoptium.net/?variant=openjdk11&jvmVariant=hotspot[the Adoptium download page].
...
* Update the version of the `maven_rt_plugin_config-master` in your `pom.xml` files to the latest `22.0.x` release. See https://search.maven.org/classic/#search%7Cgav%7C1%7Cg%3A%22org.eclipse.scout%22%20AND%20a%3A%22maven_rt_plugin_config-master%22[Maven central] for a list of versions available.
...
=== Update of 3rd Party library _chart.js_ to 3.6.0
...
----
* Replace dependencies to `babel-eslint` with these two:
+
[source,json lines]
----
"@babel/core": "7.16.0",
...
This helps to clarify and stabilize the API (the datatype does not suddenly change if the numbers get bigger) and it helps to map the value from the json to a more precise Java representation (a `Double` cannot hold all floats exactl enough).
...
The methods `not`, `takeWhile`, and `iterate` were removed; the equivalent Java 11 core library methods may now be used directly.
