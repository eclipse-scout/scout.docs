...
|&mdash; |SmartField2 _(new implementation)_ |SmartField _(new implementation)_
|===
...
== New convenience methods for setMandatory, setStatusVisible, setFieldStyle, setDisabledStyle on IFormField

There are new methods for setting mandatory-state, status-visibility, field-style and disabled-style that allow to specify if child form fields should be changed as well.
...
Scout can now detect the Web Fonts (*.woff files) to preload automatically. It's therefore no longer necessary to list the font names manually in the boostrap argument of `scout.App`.
