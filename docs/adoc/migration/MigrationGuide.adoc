...
Because the former .css files actually were LESS files, we've changed the wrong file extension from .css to .less. This allows editors with
LESS support/validation to properly work with the LESS syntax and simplifies the usage of the LESS @import clause, since the (less) hint is
not required anymore.

When migrating from an older Scout version to version 6.1 you must:

* Rename all *.css files in directory /WebContent/res to *.less
* The include syntax in macro and module files has changed:
...
** Old: `<scout:stylesheet src="res/bsicrm-all-macro.css" />`
** New: `<scout:stylesheet src="res/bsicrm-all-macro.less" />`

Note: you can still import regular .css files in your *-module.less. So importing a .css file is still possible and required in some cases.
Just make sure that all stylesheets using LESS do have a *.less file extension.
