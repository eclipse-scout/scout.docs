...
. Copy the following file to the root of the .ui module: link:https://git.eclipse.org/c/scout/org.eclipse.scout.sdk.git/plain/scout-hellojs-app/src/main/resources/archetype-resources/%5F%5FrootArtifactId%5F%5F.ui/karma.conf.js?h=releases/10.0[karma.conf.js]
...
. Move your existing index.js from the WebContent folder to src/main/js in the same module and adjust it accordingly (replace the placeholder ${simpleArtifactName} with your application name).
+
[source,javascript]
.For Scout Classic
----
...
----
cd your.app.ui
./target/node/node ./target/node/node_modules/npm/bin/npm-cli.js run snapshot-version
