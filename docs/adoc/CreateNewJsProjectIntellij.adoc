...
On the left hand side select the `Scout` type.
...
image::{imgsdir}/sdk_new_projectjs_ij.png[]
...
image::{imgsdir}/sdk_new_projectjs_location_ij.png[]
...
image::{imgsdir}/sdk_new_projectjs_initial_helloworld_ij.png[]
...
In the dialog expand the `Application` type on the left-hand-side, select the prepared run configuration `Setup local dev database` and confirm with btn:[Ok].
...
image::{imgsdir}/sdk_dbsetup_ij.png[]
...
You could start each step separately, but for the sake of convenience there is a compound run configuration available which performs all these tasks.
...
IMPORTANT: The JavaScript build could fail in case the installed Node.js could not be found or is too old.
...
image::{imgsdir}/run_helloscoutjs_full_stack_in_browser.png[]