...
The UiServlet checks after the creation of a session cookie if the server is configured safely. The validation is implemented in the Bean `org.eclipse.scout.rt.ui.html.SessionCookieValidator` and can be replaced or customized by developers.
