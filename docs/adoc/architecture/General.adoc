...
If the validation fails, the validator assumes that the session has been tampered and invalidates the session immediately. For the end user this leads to a session timeout message right after application access or that the user cannot login at all. In this case the application should be taken offline and the configuration should be fixed.
