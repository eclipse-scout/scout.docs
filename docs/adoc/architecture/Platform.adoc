...
This factory receives the bean that should be decorated and the originally requested bean class to decide which decorators that are created. In case no decoration is required the factory may return `null`. Then the original bean is used without decorations.
...
To achieve this a bean decoration factory on client side checks for availability of this annotation and creates a decorator that forwards the operation to the server.
