...
The Scout bean manager is dynamic registry for beans. Beans are normal Java classes usually having some meta data describing the characteristics of the bean.
...
This can be influenced with the `org.eclipse.scout.commons.annotations.@Order` and `org.eclipse.scout.commons.annotations.@Replace` annotations.
...
Please note that the `@Order` annotation is not inherited so that each bean must declare its own value where it fits in. If a bean does not declare an order value, the default of `5000` is used.
Scout itself uses orders in the range 4’001 – 4’999.
...
Bean decorations allow to wrap interface beans with a proxy to intercept each method call to that bean and apply some custom logic.
For this a `IBeanDecorationFactory` has to be implemented. This is one single factory instance for the entire application. It decides which beans are decorated with which interceptors.
As bean decoration factories are beans itself it is sufficient to create an implementation of `org.eclipse.scout.rt.platform.IBeanDecorationFactory` and to ensure this implementation is used (see <<Bean Retrieval>>).
This factory receives the bean and the original query class to decide which interceptors that are created.

The sample in listing <<lst-BeanDecorationFactory>> wraps each call to the server with a profiler interceptor that measures how long a server call takes.
...
To achieve this a bean decoration factory on client side checks for availability of this annotation and creates an interceptor that forwards the operation to the server.

It is best practice to mark all annotation that are interpreted in the bean decoration factory with the annotation `org.eclipse.scout.rt.platform.@BeanInvocationHint`. However this annotation has no effect at runtime and is only for documentation reasons.
...
[source,java,indent=0]
.A configuration property of type string.
----
include::{codedir}/org.eclipse.scout.docs.snippets/src/main/java/org/eclipse/scout/docs/snippets/PlatformSnippet.java[tags=ConfigProperties]
----
...
Scout applications automatically contain an inventory with the classes available and some meta data about them. This allows to find classes available on the classpath by certain criteria:

* All subclasses of a certain base class (also known as type hierarchy)
* All classes having a specific annotation.

This class inventory can be accessed using the code in listing <<lst-ClassInventory>>.
...
The class inventory is created for all Scout jars. It can also be created for custom projects by adding an empty xml file named `scout.xml` into the META-INF folder of the custom project (which is recommended).
