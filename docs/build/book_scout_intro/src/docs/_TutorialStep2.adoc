...
Using the implementation provided in <<lst-contacts_country_lookup>> create class `CountryLookupCall` in package `org.eclipse.scout.contacts.client.common`.  
...
* Select the newly created package `org.eclipse.scout.contacts.client.common` in the Package Explorer
...
----
include::{codedir}/contacts/org.eclipse.scout.contacts.client/src/main/java/org/eclipse/scout/contacts/client/person/PersonPage.java[tags=CountryColumn]
----
