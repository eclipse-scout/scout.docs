...
The first WAR file contains the Scout server with all business logic.
The second WAR file contains the user interface for desktop and mobile devices.

To start the build right click on the project [java]+org.eclipsescout.helloworld.parent+ and select the context menu menu:Run As -> Maven install[] as shown in <<img-sdk_export_war>>.

[[img-sdk_export_war, Figure 000]]
.Starting the Maven build.
image::{imgsdir}/sdk_export_war_menu.png[]

Afterwards the compilation starts, executes all test cases and bundles the result into two WAR files. The output of the build is shown in the Console view within Eclipse. In this output you find the location where the created WAR files are stored.

You should see something like "`Installing C:\Projects\Scout\HelloWorld\org.eclipsescout.helloworld.ui.html.app.war\target\org.eclipsescout.helloworld.ui.html.app.war-1.0.0-SNAPSHOT.war to C:\Users\<your_username>\.m2\repository\org\eclipsescout\helloworld\org.eclipsescout.helloworld.ui.html.app.war\1.0.0-SNAPSHOT\org.eclipsescout.helloworld.ui.html.app.war-1.0.0-SNAPSHOT.war`".

A similar output can be found some lines above for the server WAR file.
...
After picking your just built [filename]+org.eclipsescout.helloworld.server.app.war-1.0.0-SNAPSHOT.war+ and closing the file chooser, click on button "`Deploy`" (located below button "`Choose File`") to deploy the application to the Tomcat web server.
Then we repeat this step with the second WAR file [filename]+org.eclipsescout.helloworld.ui.html.app.war-1.0.0-SNAPSHOT.war+.
...
----
  http://localhost:8080/org.eclipsescout.helloworld.ui.html.app.war-1.0.0-SNAPSHOT/
----
...
Please note: In a productive environment it is recommended to deploy the server and the user interface into two different servlet containers (like Tomcat) running on dedicated machines. This because these two tiers have different requirements on resources, load balancing and access protection.
Furthermore it is hardly recommended to use an encrypted connection (e.g. TLS 1.2 footnote:[TLS: https://en.wikipedia.org/wiki/Transport_Layer_Security]) between client browser and user interface AND between user interface and server!
