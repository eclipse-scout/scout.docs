...
A more elaborate version of the same application is available as a Scout sample application on https://github.com/BSI-Business-Systems-Integration-AG/org.eclipse.scout.docs/tree/releases/6.0.x/code/contacts[Github].
...
The remaining sections focus on the individual steps to implement the "`Contacts`" application.
...
Alternatively, you can pull a Docker image of the "`Contacts`" application from Docker Hub.footnote:[
"`Contacts`" on Docker Hub: https://hub.docker.com/r/eclipsescout/org.eclipse.scout.contacts/
].
...
Before we start to add components for the "Contacts" application we will remove elements that we will not need and perfom some refactoring/renaming of the generated source code.

We start with removing all `*.helloworld` packages from the client, server and shared modules. 
As shown in <<img-contacts_package_explorer>> we have to delete footnote:[
To delete a package select the package with the mouse and hit the `Delete` key.
]
two `org.eclipse.scout.contacts.client.helloworld` packages in the client module as related code is found in `src/main/java` and `src/test/java`. 

In the server module delete the `org.eclipse.scout.contacts.server.helloworld` packages in `src/main/java` and `src/test/java`, and in the shared module
delete package `org.eclipse.scout.contacts.server.helloworld` in folder `src/main/java`. 

Next, we perform a number of modifications to class WorkOutline.
...
Let us start with the class rename.
...
In the result list, we select the desired class and click btn:[OK] which opens the file `WorkOutline.java` in the Java editor.
...
In the Java editor we can then rename the class by clicking into the class identifier `WorkOutline` and using menu menu:Refactor[Rename...].
Inside the edit box we can then change the class name to `ContactOutline` and press btn:[Enter].

Next, we rename the package `org.eclipse.scout.contacts.client.work` to `org.eclipse.scout.contacts.client.contact` by clicking into the package name and using the rename menu menu:Refactor[Rename...] again.

Finally, we change the outline title in method `getConfiguredTitle` to "Contacts" using the method described in <<sec-add_text_entry>>.
