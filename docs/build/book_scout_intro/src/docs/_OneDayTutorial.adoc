...
The Scout [wizard]_New Page Wizard_ then creates an initial implementation for the `PersonPage` class. 
Before we start to add the columns to the table of the page we need to modify the source code according to <<lst-contacts_PersonPage>>.
...
// add columns
id column (source code)
first name column (source code)
other columns just list with properties

[[sec-contacts_adding_company_page]]
==== Adding the Company Page

// high level description with attributes
...
An additional Scout topic that is touched in this [java]+ServerApplication+ class is the Scout logging. Scout uses http://slf4j.org[SLF4J] for logging.
As shown in <<lst-mycontacts.server.serverapplication>>, a static [java]+logger+ object is created using SLF4J's [java]+LoggerFactory+ class.
...
Further information regarding logging in Eclipse Scout is available on the SLF4J home page and in the Scout wiki footnote:[
