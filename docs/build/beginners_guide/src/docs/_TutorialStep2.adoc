...
We can now add the additional columns `FirstNameColumn`, `FirstNameColumn`, `LastNameColumn`, `CityColumn` below. 
...
After the country column we add the three columns `PhoneColumn`, `MobilePhoneColumn`, `EmailColumn` and `OrganizationColumn` that are initially not visible in the user interface. 
...
When comparing the state of the "Contacts" tutorial application with the Scout demo application in <<img-app_contacts_PersonTable>> the main difference is the missing person data. 
