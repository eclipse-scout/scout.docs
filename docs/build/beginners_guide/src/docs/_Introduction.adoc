...
Scout applications are Single Page Applications that run in the browser on the desktop, on tablet and on mobile devices.

==== Problem Statment

Today, the Java platform is widely seen as the primary choice for implementing enterprise applications. 
For the implementation of the enterprise backend many successful Java frameworks exist. 
On the frontend side the situation is challenging as many competing user interface technologies exist.
This is particularly true for web technologies where it is hard to predict which frameworks will remain relevant in the coming years. 

The implementation of business applications requires a substantial investment of time and money and most organizations have an interest in the protection of this investment. 
Business application life spans of 10 or more years match badly with the average life spans of web frameworks. 
Implementing substantial amounts of code against web frameworks with short lifespans and/or an uncertain future is to be avoided in this context. 
Abstracting common user interface concepts away from any specific UI technology therefore helps to reduce the risk of the frontend code becoming legacy within a short period of time. 
