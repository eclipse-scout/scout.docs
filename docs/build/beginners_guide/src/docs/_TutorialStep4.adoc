...
Depending on the content of the address fields the city and/or the country field is set to mandatory. 
Before the user can save a form the form validation will ensure that all mandatory fields are filled in. 
...
To avoid a copy&paste approach we will refactor the picture field of the person form into a template field that can be used by both the person and the company form. 
...
For the first step we navigate to the shared module into the folder `src/generated/java` of the "Contacts" application.
We then add package `org.eclipse.scout.contacts.shared.common` and a new class `AbstractUrlImageFieldData` as shown in <<lst-contacts_empty_form_data>> to this package.
...
In package `org.eclipse.scout.contacts.shared.common` of the client module we create the new template field class `AbstractUrlImageField`.
The implementation for the class is provided in <<lst-contacts_refactor_picture>>. 

With the annotation `@FormData` we provide the reference to the corresponding field data class `AbstractUrlImageFieldData` and instruct the Scout tooling to automatically update this class for us.  
...
The last step of refactoring the picture field is to replace the original code of the picture field with the newly created template field. 
As the functionality of the template field exactly matches with the original code, class `PictureField` only needs to extend the template field as shown in <<lst-contacts_picture_exends_urlimagefield>>. 
...
Using the refactored template field we want the user to be able to enter and update the URL of the shown picture.
...
Before clicking btn:[Finish] verify that you use the correct source folder and package name. 
In the [element]_Name_ field enter "PictureUrl" and make sure to deselect all checkboxes in section [element]_Additional Components_.   
...
Now adapt the content of the URL form according to <<lst-contacts_refactor_picture>>. 
...
We can now add an "Edit URL" menu to the picture template field using the picture URL to let the user edit the URL in the user interface. 
...
To reduce the amount of copy & paste for the implementation of the company form in the next section, we recommend that you may also refactor the following fields into templates.  

* EmailField
* AddressBox
* NotesBox

In the next section we are going to implement a form to enter and edit companies where we can re-use the template fields that we have created. 
...
In the fourth step of the "Contacts" tutorial we have added the Scout forms to edit and create persons and organizations. 
Based on these examples we have learned how to implement complex form layouts using the Scout layouting mechanism, Scout container fields and some individual field properties.    
We have also seen how we can use context menus to integrate the forms in the user interface of the application. 

Two additional and important topics have also be covered in this tutorial step.
