...
For the first part of the tutorial we start with a fresh install of the "Eclipse for Scout developers" package.

==== Start the Eclipse IDE

Start your Eclipse IDE and select an empty directory for your workspace in the [wizard]_Eclipse Launcher_ dialog.
In case the Eclipse welcome screen is shown, click it away.
...
WARNING: As long as the Eclipse Scout project is not integrated in the default catalog, you need to import the setup file manually.

To import the setup file manually click on the green plus icon btn:[Add user projects] which opens dialog [wizard]_Add User Projects_ as shown in <<img-oomph_add_user_projects>>.
...
In this dialog fill in the elements provided below. 

* For field `Catalog` select option "`Eclipse Projects`"
* For field `Resource URIs` enter our our Setup File: `https://git.eclipse.org/c/scout/oomph.git/plain/Scout.setup`

To close the dialog click the btn:[OK] button.
The [wizard]_Eclipse Importer_ should then contain the folder hierarchy [folder]_Eclipse Projects_, [folder]_<User>_, [folder]_Scout_ and [folder]_Scout Demo App_ as shown in <<img-oomph_exiting_projects1>>.
...
* `JRE 1.7 Location`: Enter the path to your Java 7 JRE
* `JRE 1.8 Location`: Enter the path to your Java 8 JRE
...
After the successful installation Oomph asks to restart the Eclipse IDE as shown in <<img-oomph_restart>>.

[[img-oomph_restart, Figure Restart after installation]]
.Restart of the IDE after installation.
image::{imgsdir}/oomph_restart.png[]

To complete the installation click btn:[Finish] which will restart the IDE. 
During the restart confirm the workspace shown in the [wizard]_Eclipse Launcher_ dialog.
...
You can browse through the data, open different forms and add/change entitites.
...
----
include::{codedir}/contacts/org.eclipse.scout.contacts.client/src/main/java/org/eclipse/scout/contacts/client/contact/ContactOutline.java[lines=21..50]
----
