...
The Oomph installer first makes sure that all necessary Scout componets and M2E connectors are available. 
In case one or more components are missing, Oomph will first download and install these components which triggers a restart request as shown in <<img-oomph_restart>>.
