...
** Enter "Person" into the [field]_Name_ as shown in <<img-contacts_create_new_person_form_Intellij>> and then hit btn:[Enter]
. For Eclipse:
...
** Enter "Gender" into the [field]_Name_ as shown in <<img-contacts_tutorial_new_gender_codetype_Intellij>> and then hit btn:[Enter]
. For Eclipse:
...
image::{imgsdir}/contacts_tutorial_create_new_gender_codetype_Intellij.png[]
[[img-contacts_tutorial_new_gender_codetype_Intellij]]
.Create the gender code using the Scout new code wizard in IntelliJ.
image::{imgsdir}/contacts_tutorial_new_gender_codetype_Intellij.png[]
...
----
include::{codedir}/contacts/org.eclipse.scout.contacts.server/src/main/java/org/eclipse/scout/contacts/server/security/ServerAccessControlService.java[tags=access_control]
----
