...
Additional reasons on why Scout helps to drive down cost and risks are discussed in <<sec-why_scout>>.
...
We now propose to start downloading and installing Scout as described in <<apx-install_scout>> and do some actual coding.
To do so, please continue with the "`Hello World`" example provided in <<cha-helloworld>>.
 ...
 * <<cha-helloworld>> `"Hello World"` Tutorial.
 ...
 * <<sec-initial_helloworld>> "`Walking through the Initial Application`"
 ...
 * <<cha-large_example>> "`The My Contacts Application`".
...
To learn about Scout and about its benefits first go through <<sec-what_is_scout>> and <<sec-why_scout>>.
Then, flip through <<sec-my_contacts_guide>> to get an impression of the "`My Contacts`" application.
...
Rather than including such background material in this tutorial, this information is provided separately in <<cha-helloworld_background>>.
...
If you don't know the correct username or password you may look it up in the file [filename]+tomcat-users.xml+ as described in <<apx-tomcat_dirs_and_files>>.
...
[[img-helloworld_running_clients]]
...
In addition to <<cha-helloworld>>, we include <<sec-initial_helloworld>> to discuss the initial application generated by the Scout SDK.
...
As we have seen in <<sec-helloworld_export>>, this project alias is used by the Scout SDK to build the base names of the WAR files in the export step.
In turn, after deploying the WAR files as described in <<sec-helloworld_deploy>>, the RAP server application becomes available under the URL [java]+http://localhost:8080/helloworld+.
...
----
TODO [7.0] jbr: include was {codedir}/helloworld/org.eclipse.scout.helloworld.client/src/org/eclipse/scout/helloworld/client/ui/desktop/Desktop.java[lines=32..41]
----
...
(((ViewHandler)))
[[lst-helloworld.viewhandler]]
[source,java]
.Class [java]+DesktopForm+ with its view handler and [java]+startView+ method. Other inner classes and methods are omitted here.
----
TODO [7.0] jbr: include was {codedir}/helloworld/org.eclipse.scout.helloworld.client/src/org/eclipse/scout/helloworld/client/ui/forms/DesktopForm.java[lines=19..20;86..103]
----
...
To run a Scout application the Scout SDK provides launcher boxes in the Scout Object Properties as described in <<sec-run_initial>>.
...
The four applications, one for the server application and one for each client technology, have already been discussed in <<sec-run_initial>>.
...
(((MainBox)))
[[lst-helloworld.mainbox]]
[source,java]
.The [java]+DesktopForm+ with its inner class [java]+MainBox+ containing the desktop box and message field
----
TODO [7.0] jbr: include was {codedir}/helloworld/org.eclipse.scout.helloworld.client/src/org/eclipse/scout/helloworld/client/ui/forms/DesktopForm.java[lines=18..20;69..84;102..103]
----

As discussed in <<sec-helloworld.form>> Scout forms consist of variables, the main box and a number of form handlers.
...
[[lst-helloworld.textproviderservice]]
[source,java]
...
----
TODO [7.0] jbr: include was {codedir}/helloworld/org.eclipse.scout.helloworld.shared/src/org/eclipse/scout/helloworld/shared/services/common/text/HelloworldTextProviderService.java[lines=5..10]
----
...
See <<sec-nls_editor>> for a detailed description of the NLS editor.
]
...
(((Text Provider Service)))
[[lst-helloworld.desktop_service]]
[source,java]
.The server service class [java]+DesktopService+.
----
TODO [7.0] jbr: include was {codedir}/helloworld/org.eclipse.scout.helloworld.server/src/org/eclipse/scout/helloworld/server/services/DesktopService.java[lines=8..15]
----
...
[[lst-helloworld.server_plugin_xml]]
[source,xml]
...
----
TODO [7.0] jbr: include was {codedir}/helloworld/org.eclipse.scout.helloworld.server/plugin.xml[lines=1..6;22..27;105..106]
----
...
(((Plugin service registration)))
[[lst-helloworld.client_plugin_xml]]
[source,xml]
...
----
TODO [7.0] jbr: include was {codedir}/helloworld/org.eclipse.scout.helloworld.client/plugin.xml[]
----
...
TODO [7.0] jbr: Describe what to change to use RAP default look and feel
...
TODO [7.0] jbr: Pointer to more info regarding mobile devices.
...
See <<apx-osgi_basics>> for more information regarding server-side Equinox.
],
...
See <<apx-javaee_basics>> for more information regarding servlets.
],
directory [filename]+eclipse+ contains all necessary artefacts for servlet based Eclipse applications.footnote:[
See <<apx-osgi_basics>> for more information regarding server-side Eclipse applications (server-side Equinox).
].
...
From top to bottom we find the configuration file [filename]+config.ini+ introduced in <<sec-config_ini>>.
...
These "`Hello World`" jar files exactly match with the plugin projects discussed in <<sec-create_project_simple_background>>.

[[img-helloworld_server_plugin]]
...
[[lst-helloworld.server_plugin_xml.resourceservlet]]
[source,xml]
.The configuration of the server's resource servlet in the [java]+plugin.xml+ configuration file. The remaining content of the file has been omitted.
----
TODO [7.0] jbr: include was {codedir}/helloworld/org.eclipse.scout.helloworld.server/plugin.xml[lines=1..3;28..30;51..62;104..106]
----


In <<img-helloworld_server_plugin>> some of the content of the "`Hello World`" server plugin is shown.
The first thing to note is that the plugin file conforms to the JAR file format including a [filename]+META-INF/MANIFEST.MF+ file and the directory tree containing the Java class files, as the [filename]+DesktopService.class+ implemented in <<sec-helloworld.server>> of the "`Hello World`" tutorial.
...
[[img-helloworld_server_explorer]]
...
If you have installed Tomcat on a Windows box according to <<apx-install_tomcat>> it will be running as a service.
