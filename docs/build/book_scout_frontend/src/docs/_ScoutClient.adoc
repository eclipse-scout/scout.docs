...
The code type class and its codes shown in Listing <<lst-codetype.simple>> have been created using the creation wizards provided by the Scout SDK as described in <<sec-wizard_code_type>>.
...
This is exactly the scenario that was used in the "`My Contacts`" application of the book's first part in <<sec-adding_the_smartfield>>.
...
In the text below the organisation of the widget demo application is first described in <<sec-widgetdemo_ui>>.
And in <<sec-widgetdemo_architecture>>, the setup in the form of a Scout client only application is explained.
...
In the text below the client only setup of the widget demo application is explained by looking at the main differences to the setup of the "`Hello World`" application introduced in <<cha-helloworld>>.

(((ClientSession,execLoadSession)))
[[lst-demoapp.clientsession]]
[source,java]
...
Separate buttons are available to provide sample values and to demonstrate the formatting options for displaying date and time values.
...
(((AbstractRadioButtonGroup,Radio Button Group)))
[[lst-field.radiobuttongroup.styled]]
[source,java]
