...
After copying these classes, replace the contents of
`SuperUserRunContextProducer`
with the implementation shown in
<<lst-contacts_tutorial_super_user_run_context_producer>>.

The implementation from the Contacts demo application still uses APIs related
to the backend server session. These APIs have changed in Scout 26.2, therefore
the class must be updated as shown below.

[[lst-contacts_tutorial_super_user_run_context_producer]]
[source,java,opts=novalidate]
.Replacement implementation of `SuperUserRunContextProducer`
----
...
public class SuperUserRunContextProducer {
  ...
  private final FinalValue<String> m_sessionId = new FinalValue<>();

  @PostConstruct
  protected void initSuperUserSubject() {
    m_subject.set(CONFIG.getPropertyValue(SuperUserSubjectProperty.class));
    m_user.set(BEANS.get(IAccessControlService.class).getUser(m_subject.get()));
    m_sessionId.set(SessionId.randomSessionId());
  }

  /**
   * Produces a new {@link ServerRunContext} with superuser rights.
   * <p>
   * This implies a new {@link ServerRunContext} initialized with the superuser's {@link Subject} and {@link User},
   * with a dedicated {@link RunMonitor} set, and the superuser's shared {@link SessionId}.
   */
  public ServerRunContext produce() {
    return ServerRunContexts.empty()
        ...
        .withUser(getUser())
        .withThreadLocal(SessionId.CURRENT, m_sessionId.get());
  }
