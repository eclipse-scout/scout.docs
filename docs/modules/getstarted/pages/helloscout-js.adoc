...
For this tutorial you need to have https://nodejs.org/dist/latest-v20.x/[Node.js 20] installed.
If you have multiple node versions installed, make sure the correct version is used by running the following command in your terminal.
...
If this does not print the required version, you need to adjust your PATH variable (replace NODE_DIR with your installation directory).
...
[subs=attributes+]
----
git clone https://github.com/bsi-software/helloscoutjs.git --branch releases/{scout-version}
----
...
Change the directory to the main folder where the file `package.json` is located and execute `**pnpm install**`.
If this command fails, you likely don't have pnpm installed.
In that case execute `**npm install**` instead.
...
as well as a file `package-lock.json` resp. `pnpm-lock.yaml` when using pnpm, listing all the specific versions of these dependencies.
