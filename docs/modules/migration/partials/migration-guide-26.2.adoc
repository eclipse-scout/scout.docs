...
Returning `true` in a visitor used for `Tree.visitNodes` now stops visiting completely instead of just skipping the subtree, see also xref:releasenotes:release-notes.adoc#tree-scout-js-findnode-added-and-visitnodes-improved[Release Notes].
...
== Default Minimum Password Length Increased

The default value of `scout.passwordPolicy.minLength` has changed from `12` to `14`.

If your application relies on the previous default value, configure it explicitly:

[source,properties,opts=novalidate]
----
scout.passwordPolicy.minLength=12
----

Review your password policy configuration and adjust the value if a different minimum password length is required.
