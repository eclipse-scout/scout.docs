...
If you use a `web.xml`-based deployment, make sure to add a _<multipart-config>_ section to your `web.xml` where the `UiServlet` is registered:
...
It's highly advised to update your project structure as newly defined.
...
No migration required, automatically registered by the Scout application.
...
** `scout.auth.cookieSessionValidateSecure` (existing property) is used in the `UiServlet` for validation of HTTP only and secure flags.
Make sure to set accordingly, if any of the other 2 properties are set to false
