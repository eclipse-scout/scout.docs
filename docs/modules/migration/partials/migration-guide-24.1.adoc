...
public static class UiServletContributor implements IServletContributor {
  ...
  public void contribute(ServletContextHandler handler) {
    ...
    servletHolder.getRegistration().setMultipartConfig(CONFIG.getPropertyValue(UiServletMultipartConfigProperty.class)); <1>
  }
}
----

<1> Make sure to add the multipart config.
...
Continuing this example, if additionally to 'Child Node 1' also 'Child node 2' gets checked,
the parent node is checked automatically. *Attention: The mechanism of auto-checking the parent is new.
It may break the logic in your application.*
...
Before this change, the auto-check logic was implemented in Java and JavaScript. In the
`org.eclipse.scout.rt.client.ui.basic.tree.AbstractTree#execAutoCheckChildNodes` implemented the logic
on Java side. This method was removed. The complete auto-check logic is now on JavaScript side.
