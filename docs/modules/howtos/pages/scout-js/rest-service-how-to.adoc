...
----
import {ajax, BaseDoEntity, NumberColumn, ObjectOrModel, PageWithTable, PageWithTableModel, systems, Table, TableRow, typeName} from '@eclipse-scout/core';
...
// <6>
@typeName('hellojs.PizzaResponse')
export class PizzaResponse extends BaseDoEntity {
  items: PizzaDo[];
}

 // <7>
@typeName('hellojs.Pizza')
export class PizzaDo extends BaseDoEntity {
  size: number;
  price: number;
}
...
// <8>
@typeName('hellojs.PizzaRestriction')
export class PizzaRestrictionDo extends BaseDoEntity {
  size: number;
  price: number;
}
----
...
<6> The declaration of the PizzaResponse on client side. Attribute names and typeName must correspond the PizzaResponse class in Java.
<7> The declaration of the PizzaDo on client side. Attribute names and typeName must correspond the PizzaDo class in Java.
<8> The declaration of the PizzaRestrictionDo on client side. Attribute names and typeName must correspond the PizzaRestrictionDo class in Java.

[#page-with-table-search-form]
== Create a search form

[source,typescript]
.hellojs.ui/src/main/js/pizza/PizzaSearchForm.ts
----
import {Form, FormModel, GroupBox, NumberField, ResetMenu, scout, SearchMenu} from '@eclipse-scout/core';
import {PizzaRestrictionDo} from './PizzaTablePage';

export class PizzaSearchForm extends Form {
  ...
  override importData() { // <3>
    this.widget('SizeField').setValue(this.data?.size);
    this.widget('PriceField').setValue(this.data?.price);
  }

  override exportData(): PizzaRestrictionDo { // <4>
    return scout.create(PizzaRestrictionDo, {
      size: this.widget('SizeField').value,
...
<1> A small inline model for the `PizzaSearchForm` having a search field for the size and the price.
<2> Adding a `SearchMenu` and `ResetMenu` automatically reloads the table when these menus are clicked.
<3> Importing the data: sets the values of the `PizzaRestrictionDo` on the fields.
<4> Exporting the data: creates a `PizzaRestrictionDo` using the values of the fields.
...
<1> Adding the `PizzaSearchForm` to the inline model of the `PizzaTablePage`. It is set as the form of a `SearchFormTableControl`.
Each time the table page loads its data it exports the data of the search form and uses it as search filter.
