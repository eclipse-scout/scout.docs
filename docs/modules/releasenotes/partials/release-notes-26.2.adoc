...
image::common:date_column.png[width=500]
...
Creation of `FileChooser` instances is now centralized through the new `FileChooserFactory`.

Instead of instantiating a `FileChooser` directly using `new FileChooser(...)`, applications can obtain a file chooser via:

[source,java,opts=novalidate]
----
BEANS.get(FileChooserFactory.class).createFileChooser();
----

This provides a single extension point for customizing and replacing file chooser creation.
