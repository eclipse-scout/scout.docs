...
JavaScript and CSS assets of a typical Scout application are built by  https://webpack.js.org/[Webpack] using https://pnpm.io/[pnpm] and https://nodejs.org/[Node.js]. In order to make the building as easy as possible for you, there is a https://www.npmjs.com/package/@eclipse-scout/cli[CLI] module available. That module contains a default webpack and karma configuration and several build scripts you can use. The goal is to reduce the time you need to set up your build to a minimum. If you have created your Scout project using a Scout archetype, it should all be already setup for you. Nevertheless, you will get to a point where it is important to know how the building works in detail and how the several build tools are wired together. If you are there, this chapter should help you out.

== Dependency Management

In every modern application you will have dependencies to other modules, either modules you created to separate your code, or third party modules like Scout. Such dependencies to other JavaScript modules are managed by https://pnpm.io/[pnpm]. So every module containing JavaScript or Less code needs to be a Node module with a `package.json` file that defines its dependencies.

This setup gives you the possibility to easier integrate and update 3rd party JavaScript frameworks available in the huge https://www.npmjs.com/[npmjs registry].

Scout itself is also published to that registry and will therefore be downloaded automatically once you execute `pnpm install`, as long as your `package.json` contains a Scout dependency. You will recognize a Scout module based on its name: all official Scout modules are published using the scope `@eclipse-scout`. The most important one is https://www.npmjs.com/package/@eclipse-scout/core[@eclipse-scout/core] which contains the core runtime functionality. Other modules are https://www.npmjs.com/package/@eclipse-scout/cli[@eclipse-scout/cli] for the building support, https://www.npmjs.com/package/@eclipse-scout/eslint-config[@eclipse-scout/eslint-config] for our ESLint rules, or https://www.npmjs.com/package/@eclipse-scout/karma-jasmine-scout[@eclipse-scout/karma-jasmine-scout] for enhanced testing support.

[#node-dependency-resolution]
=== Dependency Resolution

https://pnpm.io/[Pnpm] takes care to link all dependencies declared in a `package.json` file to a `node_modules` directory next to it. If it is a 3rd party dependency, it is downloaded to a local store before.
...
When a dependency is installed, pnpm decides which version to use based on the declared SemVer constraint.
...
Furthermore, it increases the risk of supply chain attacks, where an attacker compromises an npm account and publishes a malicious package that will be downloaded automatically.
...
The generated overrides contain all dependencies and transitive dependencies that are declared using a ranged non-snapshot version.

While the exact version in the overrides depends on the available versions at the time you execute the install script, the following examples give an idea how the overrides are generated:

* Declaring a dependency `"@eclipse-scout/core": "26.1.16"` will result in no override as the version is already fixed.
* Declaring a dependency using a ranged version like `"@eclipse-scout/core": "~26.1.0"` will result in an override like `"@eclipse-scout/core": 26.1.16`.
* Declaring the same dependency in different packages using different ranges like `"@eclipse-scout/core": "~26.1.0"` in `pck-a` and `"@eclipse-scout/core": ">=25.2.0 <26.1.0"` in `pck-b` will result in overrides like `"@eclipse-scout/core": 26.1.16` and `pck-b>@eclipse-scout/core: 25.2.20`.
* Declaring a dependency using a snapshot version like `"@eclipse-scout/core": ">=26.2.0-snapshot <26.2.0"` will result in no override as snapshot versions will not be fixed.

The `install` command of the `@eclipse-scout/releng` module has the following parameters:

* `updateMode`: Specifies which dependencies should be updated during install. Default is `required`. Must be one of the following:
** `required`: Update dependencies that do not fulfill the version declaration in the `package.json`.
This should be used during development so that the overrides are adapted as required in case you modify the dependencies in the `package.json`.
Running install using this mode is more or less the same as performing `pnpm install` without the previously generated overrides and re-generating new overrides afterward.
Therefore, only those overrides are updated that do no longer satisfy changed version constraints in your `package.json`.
** `all`: Update all dependencies to the newest possible version that currently exists and matches the version specifiers of the `package.json`.
This can be used if you want to perform a manual full update of the whole dependency tree.
The update mode `all` is nearly the same as running `pnpm update` without the previously generated overrides and re-generating new overrides afterward.
So running install using this mode leads to all overrides being updated to the newest version satisfying the version constraints from the `package.json`.
** `snapshots`: Only `snapshot` dependencies are updated.
All other dependencies use the version declared in the overrides.
As only `snapshots` can be updated, and they are never part of the overrides, the overrides will not be touched in this mode.
This mode is typically used by CI builds.
Running install using this mode will not change the overrides as it is more or less a `pnpm update` with the previously generated overrides.
Therefore, only non overridden dependencies (i.e. snapshot dependencies) are updated to their newest version satisfying the version constraints from the `package.json`.
...
** `single-external`: Log dependencies for which a single pnpm-workspace-external dependency exists.
If a dependency is used in different versions but all of your transitive dependencies are using the same version,
this level may be handy to eliminate these duplicate dependency versions by adapting the dependency versions in your workspace to the single external one.
** `own`: Log only mixed dependency versions in the own pnpm-workspace.
These may be misconfigured (old) packages.
** `none`: Nothing is logged.

If you want to disable dependency locking using overrides and restore the old (and riskier) behavior do the following:
...
.. If you want no locking at all: delete the file and add it to your VCS ignore list or add the https://pnpm.io/cli/install#--no-lockfile[--no-lockfile] argument to the script.
...
The Scout CLI is a bunch of npm-scripts that help you build and test your application. In order to use them you need to add a devDependency to https://www.npmjs.com/package/@eclipse-scout/cli[@eclipse-scout/cli] to the `package.json` of your module. We also suggest to add some scripts to make the execution easier. If you use the Scout archetype, the following will be created for you.
...
Before you can run your unit tests you need to properly set up the files as described in <<Karma Configuration>>.

If all is set up correctly, you can run your tests using the following command:
...
TIP: If you don't like the automatic browser reloading, you can press debug in the top right corner of the browser or manually navigate to http://localhost:9876/debug.html.
...
----
"devDependencies": {
  "@eclipse-scout/eslint-config": "24.2.17",
  "eslint": "9.39.2"
}
----

Then create a file called `eslint.config.mjs` with the following content:
...
The command above will analyze your current directory including all subdirectories.
...
To do that, just add the following to your `eslint.config.mjs`:
...
To do so, adjust your `eslint.config.mjs` and add the target folder to the `globalIgnores` list.
