...
[source,java]
----
public class MyApiExposedCodeTypeContributor implements IApiExposedCodeTypeContributor {

  @Override
  public void contribute(Set<ICodeType> codeTypes) {
    codeTypes.add(BEANS.get(MyCodeType.class));
  }
}
----
...
[source,java]
----
public class MyObjectTypeProvider implements IObjectTypeProvider {

  @Override
  public String objectTypeOf(Class<?> type) {
    if (MyCodeType.class.equals(type)) {
      return "project.MyCodeType";
    }
    return null;
  }
}
...
This makes `MyCodeType` available through the Scout API without requiring the original CodeType class to be replaced.
