...
According to the https://www.rfc-editor.org/rfc/rfc9110.html#name-safe-methods[Internet Standard] GET (amongst others) requests are used to read data  and must not change anything! If a REST API wants to violate the specification, such requests must be protected against https://en.wikipedia.org/wiki/Cross-site_request_forgery[CSRF] which is not enabled for GET, OPTIONS, TRACE and HEAD requests by default. See the Scout Bean `org.eclipse.scout.rt.rest.csrf.HttpMethodAntiCsrfFilterExclusion` for more details.

GET requests are idempotent, which means making multiple identical requests ends up having the same result as a single request (assuming the data has not been changed in the meantime).
...
A REST resource using the https://jakartaee.github.io/rest/[Jakarta RESTful Web Services] API is implemented by a POJO class annotated with a set of annotations.
...
All available REST resources are automatically registered by the `RestApplication` class during the Scout platform startup.

Add an implementor of `IServletContributor` in your `UiServletContributors` container class to expose your REST API using the `/api` context path (set `@Order` appropriately):

[source,java]
----
/**
 * Jakarta RESTful Web Services Jersey Servlet.
 */
@Order(20)
...
The Jakarta RESTful Web Services application API (`jakarta.ws.rs.core.Application`) allows a REST application implementation to specify a set of classes, a set of singleton instances and a map of custom properties to be registered.
...
The marshaller between Java data objects and JSON is abstracted in the Jakarta RESTful Web Services specification.
...
The filter must be added via `IServletFilterContributor` and should be configured to be called after the authentication filter (set `@Order` appropriately).
...
[source,java]
.Registration example for HttpServerRunContextFilter.
----
@Order(40)
...
Some API may be used between applications internally but it should not be made publicly available.

To distinguish between exposed and internal API the `@ApiExposed` (short for `@ApiExposed(true)`) annotation should be added to all exposed resource operation methods (or even whole resource classes).
These methods should handle input with care, e.g. ensure input is in the proper format and ensure caller is authorized and permitted to run the operation.

With the `ApiExposedFilter` restricted operations are handled:

* Request is rejected if it contains a `X-Scout-Proxied-Request` HTTP header (see `ApiExposedFilter#HTTP_HEADER_NAME`).
* Otherwise: Request is allowed

The previously mentioned HTTP header is added by:

* All `AbstractRestProxyRequestHandler` which may be used to forward requests from one incoming server to another internal server.
* It can optionally be added by a `Filter` wrapping incoming requests using an `AdditionalHeadersHttpServletRequestWrapper` on the same server.

The `org.eclipse.scout.rt.rest.resource.AbstractApiExposedFeature.ApiApiExposedFeature` is automatically contributed using a `IRestApplicationClassesContributor` and registers the `ApiExposedFilter` for all resource operations with no or an `@ApiExposed(false)` annotation.
This feature is marked with an `@RestApplicationScope(RestApplicationScopes.API)` annotation which may be used to filter it (e.g. `ApiRestApplication` includes this feature; `ExtRestApplication` filters it; regular `RestApplication` does no filtering at all)

== Dependency Management

Scout REST services based on Jakarta RESTful Web Services using the Jersey library and the Jackson JSON marshaller need a maven dependency to `jersey-media-json-jackson` in the application pom.xml.
This enables the use of Jackson as Jakarta RESTful Web Services marshaller with the Jersey Jakarta RESTful Web Services implementation.
...
----
<!-- Jakarta RESTful Web Services Jersey -->
<dependency>
...
Scout does provide a default implementation for a cancellation resource (see `org.eclipse.scout.rt.rest.cancellation.CancellationResource`).

Also a default client for this resource is provided (see `org.eclipse.scout.rt.client.rest.CancellationResourceClient`).
...
To ensure a user cancels only his own requests, the `CancellationResource` and the `RestRequestCancellationServletFilter` determines the current user
out of the currently running context (e.g. `org.eclipse.scout.rt.platform.context.RunContext.CURRENT.get().getUser()`) setup by HTTP servlet filter `org.eclipse.scout.rt.server.commons.context.HttpRunContextFilter`.
If a project does not use an HTTP servlet filter determining the current user, the corresponding `resolveCurrentUserId()` should be overwritten (and either return null if no users are available or determine the user id using another method).

[[cha-rest-multipart]]
== REST Multipart Support

Due to missing support for multipart handling in official Jakarta RESTful Web Services API, Scout provides an own implementation allowing to use multipart requests without requiring other third party libraries.
...
== REST Chunked Support

Due to missing support for responses using chunks of data (e.g. data objects) in official Jakarta RESTful Web Services API, Scout provides an own implementation allowing to use chunked responses without requiring other third party libraries.

[source,java]
.Example: Client
----
include::common:example$org.eclipse.scout.docs.snippets/src/main/java/org/eclipse/scout/docs/snippets/rest/ChunkedResourceClient.java[tags=method]
----

[source,java]
.Example: Server
----
include::common:example$org.eclipse.scout.docs.snippets/src/main/java/org/eclipse/scout/docs/snippets/rest/ChunkedResource.java[tags=method]
----
...
Scout provides a `MetaResource` which a set of endpoints offering various meta information about available endpoints.
The `MetaResource` is available at `https://<server-url>/api/meta/`

=== Version

See `MetaResource` endpoint `/version` (e.g. `https://<server-url>/api/meta/version?verbose=true`) for version information.
...
"applicationVersion": "1.2.34", <2>
"buildDate": "2024-11-20 16:01:32.000" <3>
}
----
<1> Based on config property `scout.application.name`
<2> Based on config property `scout.application.version`
<3> Based on `IModuleBuildDateProvider` implementation, which allows an application to provide its own build date.

=== Example implementation for `IModuleBuildDateProvider`


[source,java]
----
public class BuildDateProvider implements IModuleBuildDateProvider {
  @Override
  public Date getBuildDate() {
    return BEANS.get(JarManifestHelper.class).getBuildDateAttribute(getClass());  <1>
  }
}
----
<1> Reads build date from JAR manifest of module of class which is specified by `getClass()`.
