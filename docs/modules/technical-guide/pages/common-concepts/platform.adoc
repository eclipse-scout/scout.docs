...
+
If enabled the application validates that the `httpOnly` and `Secure` flags are set via `scout.app.sessionCookieConfigHttpOnly` and `scout.app.sessionCookieConfigSecure`.
+
