...
The `idSignature` flag of the annotation can be used to enable id signatures for all data objects transferred over the service tunnel for a specific interface.
For more information and examples when this is necessary see xref:technical-guide:common-concepts/security.adoc#id-signature-service-tunnel[IId signatures in service tunnel].
...
`org.eclipse.scout.rt.platform.management.EnableListenerListProfiling` ::
Defines if all IListenerListWithManagement classes are monitored and exposed via JMX beans. Enable this feature to detect memory issues or possible leaks, this may result in a global performance decrease therefore this feature is disabled by default.
+
Data type: Boolean
...
`scout.app.contextHandlerExtendedResourceLookup` ::
Specifies whether the application context handler should also lookup resources w/o base resource. The default value is false.
+
Data type: Boolean
...
Specifies whether org.eclipse.jetty.compression.server.CompressionHandler is used. Default value is true.
+
Data type: Boolean

`scout.app.gzip.excluded.inflate.paths` ::
Excluded inflate paths for org.eclipse.jetty.compression.server.CompressionHandler (respected only if at least one is set otherwise Jetty default is used).
+
Data type: List

`scout.app.gzip.excluded.methods` ::
Excluded methods for org.eclipse.jetty.compression.server.CompressionHandler (respected only if at least one is set otherwise Jetty default is used).
+
Data type: List

`scout.app.gzip.excluded.mimeTypes` ::
Excluded MIME types for org.eclipse.jetty.compression.server.CompressionHandler (respected only if at least one is set otherwise Jetty default is used).
+
Data type: List

`scout.app.gzip.excluded.paths` ::
Excluded paths for org.eclipse.jetty.compression.server.CompressionHandler (respected only if at least one is set otherwise Jetty default is used).
+
Data type: List

`scout.app.gzip.included.inflate.paths` ::
Included inflate paths for org.eclipse.jetty.compression.server.CompressionHandler (respected only if at least one is set otherwise Jetty default is used).
+
Data type: List

`scout.app.gzip.included.methods` ::
Included methods for org.eclipse.jetty.compression.server.CompressionHandler (respected only if at least one is set otherwise Jetty default is used).
+
Data type: List

`scout.app.gzip.included.mimeTypes` ::
Included MIME types for org.eclipse.jetty.compression.server.CompressionHandler (respected only if at least one is set otherwise Jetty default is used).
+
Data type: List

`scout.app.gzip.included.paths` ::
Included paths for org.eclipse.jetty.compression.server.CompressionHandler (respected only if at least one is set otherwise Jetty default is used).
+
Data type: List

`scout.app.gzip.inflateBufferSize` ::
Inflater buffer size for org.eclipse.jetty.compression.server.CompressionHandler to enable compressed requests. Default value: 2048
+
Data type: Integer >= 0

`scout.app.gzip.minSize` ::
Minimum size to trigger compression for org.eclipse.jetty.compression.server.CompressionHandler. If not set Jetty default value will be used.
+
Data type: Integer >= 0

`scout.app.gzip.syncFlush` ::
Enables sync flush for org.eclipse.jetty.compression.server.CompressionHandler. Default value is true.
+
Data type: Boolean

`scout.app.http.request.maxHeaderSize` ::
Specifies the maximum allowed size in bytes for a HTTP request header. The default value is 72 KB.
+
Data type: Integer
...
`scout.app.sessionCookieConfigPartitioned` ::
Specifies whether the HTTP session cookie created by the web application will be marked as partitioned. The default value is false.
+
Data type: Boolean
...
Specifies whether the HTTP session cookie created by the web application will be marked as secure. If true, the HTTP session cookie will be marked as secure even if the request initiated the corresponding session uses plain HTTP instead of HTTPS (e.g. Scout application behind reverse proxy terminating TLS). If false, the HTTP session cookie will not be marked as secure even if the request initiated the corresponding session is secure (using HTTPS). By default the cookie is secure for non-development code (property `scout.devMode` is false) or if the value of the property `scout.app.useTls` is true.
+
Data type: Boolean
...
The display name of the application. Used e.g. in the info form. The default value is `unknown`.
+
Data type: String

`scout.application.version` ::
The application version as displayed to the user. Used e.g. in the info form. The default value is `0.0.0`.
+
Data type: String
...
`scout.client.defaultUiCallbackTimeoutMillis` ::
Default maximum time in milliseconds to wait for the browser to respond to a UI callback request.
+
...
The maximum number of client notifications that are consumed at once. The default is 100.
+
Data type: Integer >= 0
...
Capacity of the client notification queue. If maximum capacity is reached, notification messages are dropped. The default value is 10000.
+
Data type: Integer >= 0
...
+
By default this property is enabled if the property `scout.backendUrl` is set.
+
Data type: Boolean
...
+
The values of this property replace the Scout defaults which are configured in the bean `org.eclipse.scout.rt.security.csp.ConfigurableContentSecurityPolicy`.
+
...
+
To append to configured defaults use the config property `scout.cspDirectiveAppend` instead.
+
Example:
+
[listing]
scout.cspDirective[img-src]=`self` https://media.example.com
+
Data type: Map

`scout.cspDirectiveAppend` ::
Configures additional Content Security Policy (CSP Version 2) directive expressions. The value must be provided as a Map.
+
See https://www.w3.org/TR/CSP2/ or https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/CSP for available directives and their meaning.
+
The values of this property are appended to the already existing defaults (e.g. as configured in the bean `org.eclipse.scout.rt.security.csp.ConfigurableContentSecurityPolicy` or in a config.properties file).
+
...
+
This allows to e.g. specify your default properties in the config.properties file using property `scout.cspDirective` and afterward extending it depending on a specific environment with an environment variable using this property.
+
Example:
+
[listing]
scout.cspDirectiveAppend[img-src]=https://additional.host.org
+
Data type: Map

`scout.cspEnabled` ::
Enable or disable Content Security Policy (CSP) headers. The headers can be modified by replacing the bean `org.eclipse.scout.rt.security.csp.ContentSecurityPolicy` or using the property `scout.cspDirective`.
+
Data type: Boolean
...
`scout.dataobject.jackson.streamReadConstraints` ::
Jackson constraints to use for JSON reading.
+
Map property with the keys as follows:
+
- maxNestingDepth: Sets the maximum nesting depth. The depth is a count of objects and arrays that have not been closed, `{` and `[` respectively. (default: 1000)
+
- maxDocumentLength: Sets the maximum allowed document length (for positive values over 0) or indicate that any length is acceptable (0 or negative number). The length is in input units of the input source, that is, in bytes or chars. (default: -1)
+
- maxNameLength: Sets the maximum name length (in chars or bytes, depending on input context). (default: 50000)
+
- maxNumberLength: Sets the maximum number length (in chars or bytes, depending on input context). (default: 1000)
+
- maxStringLength: Sets the maximum string length for a single attribute value of type text (in chars or bytes, depending on input context). (default: 100000000)
+

+
Data type: StreamReadConstraints

`scout.dataobject.jackson.streamWriteConstraints` ::
...
+
Data type: StreamWriteConstraints
...
`scout.http.async.forceHttp2.maxConnectionsPerRoute` ::
Specifies the default maximum connections per route property for the default async force-http2 HTTP client.
+
Default value is 2048.
+
Data type: Integer

`scout.http.async.forceHttp2.maxConnectionsTotal` ::
Specifies the default total maximum connections property for the default async force-http2 HTTP client.
+
...
`scout.http.evictExpiredConnections` ::
Enable eviction of expired connections.
+
The default value is enabled (= true)
+
Data type: Boolean

`scout.http.evictIdleConnectionsTimeout` ::
Enable eviction of idle connections with the specified number in seconds (use zero to disable).
+
...
`scout.http.proxy.async.clientManager` ::
Default client manager for HttpProxy bean; users of this bean may choose a different client manager.
+
Data type: Fully qualified class name. The class must have `org.eclipse.scout.rt.shared.http.async.AbstractAsyncHttpClientManager` in its super hierarchy.

`scout.http.proxy.async.timeout` ::
Timeout for async servlet contexts.
+
Data type: Long
...
+
Alternatively also a proxy type can be defined (must be a type of the enum java.net.Proxy$Type) using this format REGEXP_FOR_URI=PROXY_TYPE=PROXY_HOST:PROXY_PORT
+
Example:
+
[listing]
scout.http.proxyPatterns[0]=.*\.example.com(:\d+)?=127.0.0.1:8888
scout.http.proxyPatterns[1]=.*\.example.org(:\d+)?=proxy.example.com:8080scout.http.proxyPatterns[2]=.*\.example.net(:\d+)?=SOCKS=proxy.example.com:1080
+
Data type: List
...
`scout.otel.asyncObservableConcurrentJobCount` ::
The number of concurrent jobs that are executing observations asynchronously. The default value is 5.
+
Note: This value should be smaller than the number of max DB connections and there should be a fair amount of DB connections available for synchronous requests.
+
Data type: Integer >= 0

`scout.otel.asyncObservationInterval` ::
Minimum interval (in seconds) at which the observations are executed asynchronously.
+
Data type: Integer >= 0
...
`scout.otel.span.attribute.processor.enabled` ::
Property to specify if org.eclipse.scout.rt.shared.opentelemetry.OpenTelemetrySpanAttributeProcessor should add span attributes. Default is false.
+
Data type: Boolean

`scout.otel.tracing.enabled` ::
Property to specify if the application is using the Scout OpenTelemetry Tracing. Default is false.
+
Data type: Boolean

`scout.passwordPolicy.minLength` ::
Minimum length for passwords checked by org.eclipse.scout.rt.platform.security.PasswordPolicy
+
Data type: Integer >= 0

`scout.platform.version` ::
Version of the scout platform. Used e.g. in the info form. The default value is bound to the implementation version in the manifest file.
+
Data type: String
...
`scout.rest.api.exposed.enforce` ::
Set to true to reject non api-exposed operations with a not found error (404), no warning will be logged in this case (just for development mode); if set to false only a warning will be logged but requests are not rejected (default value: true)
+
Data type: Boolean

`scout.rest.client.http.connectionKeepAlive` ::
Specifies the maximum life time in milliseconds for kept alive connections of the REST HTTP client. The default value is 30 minutes.
+
Data type: Long

`scout.rest.client.http.connectionTimeToLive` ::
Specifies the maximum time to live of a HTTP connection within the HTTP connection pool. May be zero if the connection does not have an expiry deadline. The default value is 30 minutes.
+
Data type: Long

`scout.rest.client.http.forceEncodeUriForRequest` ::
Specifies whether the URI should be re-encoded to force US-ASCII encoding (strict RFC 2396 compliance), default: true
+
Data type: Boolean

`scout.rest.client.http.maxConnectionsPerRoute` ::
Configuration property to define the default maximum connections per route of the Apache HTTP client. The default value is 32
+
Data type: Integer

`scout.rest.client.http.maxConnectionsTotal` ::
Specifies the total maximum connections of the Apache HTTP client. The default value is 128
+
Data type: Integer

`scout.rest.client.http.validateAfterInactivity` ::
Defines period of inactivity in milliseconds after which persistent connections must be re-validated prior to being leased to the consumer. Non-positive value passed to this method disables connection validation. This check helps detect connections that have become stale (half-closed) while kept inactive in the pool. The default value is 1
+
Data type: Integer
...
`scout.selenium.driver` ::
Driver to use (`chrome`, `edge`, `firefox`)
+
Data type: String

`scout.selenium.driverConfig` ::
Possibility to set arbitrary selenium driver properties
+
Data type: Map

`scout.selenium.driverRemoteServerUrl` ::
If remote server URL is set, then the driver connects to selenium driver instance usually deployed with docker.
+
Data type: String

`scout.selenium.screenshotOnFailure` ::
If set, then a screenshot is taken in case of a failed test
+
Data type: Boolean

`scout.selenium.simulateSlowNetwork` ::
If set, unit tests simulate slow network
+
Data type: Boolean

`scout.selenium.webAppUrl` ::
URL to web application under test
+
Data type: String

`scout.selenium.webQueryParams` ::
Query params appended to web application under test
+
Data type: String
...
Specifies if the service tunnel should use gzip compression for requests, default value: true
+
Data type: Boolean
...
`scout.ui.servletMultipartConfig` ::
Multipart configuration for inbound servlet.
+
Map property with the keys as follows:
+
- location: the directory location where files will be stored temporarily (default: temp directory)
+
- maxFileSize: the maximum size allowed in MB for uploaded files, -1 means unlimited (default: -1 MB)
+
- maxRequestSize: the maximum size allowed in MB for multipart/form-data requests, -1 means unlimited (default: -1 MB)
+
- fileSizeThreshold: the size threshold in MB after which files will written to disk (default: 20 MB)
+

+
Data type: MultipartConfigElement
