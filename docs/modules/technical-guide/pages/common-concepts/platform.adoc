...
Set a positive number to enable incoming compressed requests (default: 2048), positive number must be set if `scout.servicetunnel.compress` property is set to `true`
+
Data type: Integer
...
+
See https://www.w3.org/TR/CSP2/ and the Bean `org.eclipse.scout.rt.security.csp.ContentSecurityPolicy` for more details.
+
...
+
The property value is of the format REGEXP_FOR_URI=PROXY_TYPE=PROXY_HOST:PROXY_PORT
+
Example:
+
[listing]
