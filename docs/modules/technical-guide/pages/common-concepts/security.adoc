...
https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/CSP[Content Security Policy (CSP)] is a feature that helps to prevent or minimize the risk of certain types of security threats.
...
Scout uses the following CSP settings:

. For the root HTML page (entry point). Uses default settings from the `ConfigurableContentSecurityPolicy` class and nonces for script tags. The following default directives are set:
+
--
.. `default-src`: `'none'`. Disables fallback handling, directives should be set explicitly.
.. `script-src`: Uses `nonce`. Only scripts having a nonce are executed.
...
.. `child-src`: `'self'`
.. `child-src`: `'self'`
.. `frame-src`: `*`. Allows all external URLs in `BrowserFields`.
.. `manifest-src`: `'self'`
.. `manifest-src`: `'self'`
.. `media-src`: `'self'`
.. `object-src`: `'self'`
...
--
+
These defaults can be modified using config property `scout.cspDirective` and `scout.cspDirectiveAppend`.
See the xref:technical-guide:common-concepts/platform.adoc#sec-app.configProperties[Scout Config Properties] for more details.
. The `AbstractBrowserField` allows configuring the CSP for each instance and `BinaryResource` (this is not possible if loading external URLs because they define their CSPs themselves). It is recommended to use specific CSP rules for such fields which should be as restrictive as possible.
+
As a starting policy the `BlockAllContentSecurityPolicy` could be used and then opening directives as necessary. The created policies can be applied to a field in e.g. the `execInitField` method by using the following methods:
+
...
. REST responses include a default `BlockAllContentSecurityPolicy`. If other CSP directives are necessary, the corresponding HTTP header can be set in the response.
. If you need to change the response of a resource (e.g. to finetune its CSP response header), the interface `IWebResourceResponseInterceptorContributor` can be implemented.
Such contributors are called for each web resource loaded and return a list of `IHttpResponseInterceptor` for a request.
...
If a resource is blocked because it violates the CSP, a report is created and logged on server side using level `info`. This is done in the class `ContentSecurityPolicyReportHandler`.
This enables admins to monitor the application and to react if a CSP violation is detected.

CSP rules can be customized with config properties (`scout.cspDirective`, `scout.cspDirectiveAppend`, `scout.cspExclusions`, `scout.cspEnabled`).
See the xref:technical-guide:common-concepts/platform.adoc#sec-app.configProperties[Scout Config Properties] for more details.
...
public static class ApiIdSignatureFilterContributor implements IServletFilterContributor {
  ...
  public void contribute(ServletContextHandler handler) {
    handler.addFilter(IdSignatureFilter.class, ServletConstants.API_PATH_WITH_WILDCARD, null);
  }
}
----
...
[#id-signature-service-tunnel]
The service tunnel supports id signatures as well, but they are not enabled by default.
To enable id signatures in the service tunnel for a specific interface use the `idSignature` flag of the `TunnelToServer` annotation (see xref:technical-guide:common-concepts/platform.adoc#tunnel-to-server[@TunnelToServer]).
This is necessary if e.g. an `IDataObject` which is known to the backend but not to the ui server is passed on to the browser where it is used for REST calls to endpoints hosted on the backend server.

[source,java]
.Enable id signatures for all calls to `ISignedService` using the `idSignature` flag of the `TunnelToServer` annotation
----
@TunnelToServer(idSignature = true)
public interface ISignedService extends IService {
}
----

If a method of an interface needs to be called with signatures or without signatures depending on the context, one can create its own proxy and pass the id signature flag in the `ServiceTunnelOptions`.

[source,java]
.Enable id signatures for a call to `IUnsignedService#someMethod()` using the `idSignature` flag of the `ServiceTunnelOptions` annotation
----
ServiceTunnelUtility.createProxy(IUnsignedService.class, ServiceTunnelOptions.create().withIdSignature(true)).someMethod();
----
