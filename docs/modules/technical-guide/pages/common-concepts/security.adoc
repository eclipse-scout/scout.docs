...
Content Security Policy is an HTTP response header that helps you reduce XSS risks by declaring what dynamic resources are allowed to load footnote:[http://content-security-policy.com/].

Scout sets the following settings by default:

* default-src 'none': Disables fallback handling, directives should be set explicitly.
* base-uri, child-src, font-src, form-action, manifest-src, media-src, object-src, script-src, worker-src 'self': Only accepts resources from the same origin (same scheme, host and port).
* style-src 'self' 'unsafe-inline': Only accepts Stylesheet resources from the same origin (same scheme, host and port). Inline style attributes are allowed.
* frame-src *: All sources are allowed because the iframes created by the Scout BrowserField run in the sandbox mode and therefore handle the security policy on their own.
...
CSP rules can be customized with config properties (`CspDirectiveProperty`, `CspEnabledProperty`, `CspExclusionsProperty`) or by replacing `ContentSecurityPolicy`.
