...
|OpenTelemetry `Context` for tracing. For more information see the chapter on link:observability.adoc[observability]
|Context.current()
|===
...
Typically, that collector is set by `ServiceTunnelService` for the processing of a service request.
...
Typically, this node ID is set by `ServiceTunnelService` for the processing of a service request.
